...
void MaterialDeferred::Update(uint32_t imageIndex)
{
    MaterialBase::Update(imageIndex);

    auto bufferInfo = VulkanDescriptorWriter().
    WriteBuffer(0, m_pMatDesc->GetSetLayout(0)->GetBindings(), &ULightUniform->GetUniformBuffer(imageIndex)->GetDscriptor()).
    Build();
