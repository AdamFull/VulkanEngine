    ...
    namespace Resources
    {
        ...
        namespace Loaders
        {
            ...
            public:
                ...
                static std::shared_ptr<Objects::RenderObject> Load(std::string srPath, std::string srName, std::shared_ptr<LoaderTemporaryObject> pTmp, std::shared_ptr<Resources::ResourceManager> pResMgr);

            private:
                static void LoadNode(std::shared_ptr<LoaderTemporaryObject> tmp, std::shared_ptr<Objects::RenderObject> pParent, const tinygltf::Node &node, uint32_t nodeIndex, const tinygltf::Model &model, float globalscale);
