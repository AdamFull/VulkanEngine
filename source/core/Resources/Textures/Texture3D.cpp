...
namespace Engine
{
    ...
    void Texture3D::LoadNoise(ENoisePattern ePattern, uint32_t width, uint32_t height, uint32_t depth)
    {
        vk::Format format;
        ktxTexture* texture;
        ImageLoader::AllocateRawDataAsKTXTexture(&texture, &format, width, height, depth, 3, GetInternalFormat());

        InitializeTexture(texture, format);

        switch (ePattern)
        {
        case ENoisePattern::ePerlin: GeneratePerlinNoise(texture); break;
        case ENoisePattern::eFractal: GenerateFractalNoise(texture, 6, 0.5); break;
        }
