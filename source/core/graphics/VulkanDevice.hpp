...
namespace Engine
{
    namespace Core
    {
        ...
        public:
            ...
            template<> vk::Result create(vk::FramebufferCreateInfo& info, vk::Framebuffer* ref) 
            { return vkDevice.createFramebuffer(&info, pAllocator, ref); }
            template<> vk::Result create(vk::SemaphoreCreateInfo& info, vk::Semaphore* ref) 
            { return vkDevice.createSemaphore(&info, pAllocator, ref); }
            template<> vk::Result create(vk::FenceCreateInfo& info, vk::Fence* ref) 
            { return vkDevice.createFence(&info, pAllocator, ref); }
            template<> vk::Result create(vk::ShaderModuleCreateInfo& info, vk::ShaderModule* ref) 
            { return vkDevice.createShaderModule(&info, pAllocator, ref); }
            template<> vk::Result create(vk::PipelineLayoutCreateInfo& info, vk::PipelineLayout* ref) 
            { return vkDevice.createPipelineLayout(&info, pAllocator, ref); }
            template<> vk::Result create(vk::GraphicsPipelineCreateInfo& info, vk::Pipeline* ref) 
            { return vkDevice.createGraphicsPipelines(pipelineCache, 1, &info, pAllocator, ref); }
            template<> vk::Result create(vk::ComputePipelineCreateInfo& info, vk::Pipeline* ref) 
            { return vkDevice.createComputePipelines(pipelineCache, 1, &info, pAllocator, ref); }
            template<> vk::Result create(vk::DescriptorSetLayoutCreateInfo& info, vk::DescriptorSetLayout* ref) 
            { return vkDevice.createDescriptorSetLayout(&info, pAllocator, ref); }
            template<> vk::Result create(vk::MemoryAllocateInfo& info, vk::DeviceMemory* ref) 
            { return vkDevice.allocateMemory(&info, pAllocator, ref); }
            template<> vk::Result create(vk::ImageCreateInfo& info, vk::Image* ref) 
            { return vkDevice.createImage(&info, pAllocator, ref); }
            template<> vk::Result create(vk::ImageViewCreateInfo& info, vk::ImageView* ref) 
            { return vkDevice.createImageView(&info, pAllocator, ref); }
            template<> vk::Result create(vk::SamplerCreateInfo& info, vk::Sampler* ref) 
            { return vkDevice.createSampler(&info, pAllocator, ref); }
            template<> vk::Result create(vk::CommandPoolCreateInfo& info, vk::CommandPool* ref) 
            { return vkDevice.createCommandPool(&info, pAllocator, ref); }
            template<> vk::Result create(vk::CommandBufferAllocateInfo& info, vk::CommandBuffer* ref) 
            { return vkDevice.allocateCommandBuffers(&info, ref); }
            template<> vk::Result create(vk::DescriptorSetAllocateInfo& info, vk::DescriptorSet* ref)
            { return vkDevice.allocateDescriptorSets(&info, ref); }
            template<> vk::Result create(vk::PipelineCacheCreateInfo& info, vk::PipelineCache* ref)
            ...
            template<> void destroy(vk::Buffer* ref)                { vkDevice.destroyBuffer(*ref, pAllocator); }
            template<> void destroy(vk::DeviceMemory* ref)          { vkDevice.freeMemory(*ref, pAllocator); }
            template<> void destroy(vk::Framebuffer* ref)           { vkDevice.destroyFramebuffer(*ref, pAllocator); }
            template<> void destroy(vk::RenderPass* ref)            { vkDevice.destroyRenderPass(*ref, pAllocator); }
            template<> void destroy(vk::SwapchainKHR* ref)          { vkDevice.destroySwapchainKHR(*ref, pAllocator); }
            template<> void destroy(vk::Pipeline* ref)              { vkDevice.destroyPipeline(*ref, pAllocator); }
            template<> void destroy(vk::PipelineLayout* ref)        { vkDevice.destroyPipelineLayout(*ref, pAllocator); }
            template<> void destroy(vk::PipelineCache* ref)         { vkDevice.destroyPipelineCache(*ref, pAllocator); }
            template<> void destroy(vk::ShaderModule* ref)          { vkDevice.destroyShaderModule(*ref, pAllocator); }
            template<> void destroy(vk::CommandPool* ref)           { vkDevice.destroyCommandPool(*ref, pAllocator); }
            template<> void destroy(vk::SurfaceKHR* ref)            { vkInstance.destroySurfaceKHR(*ref, pAllocator); }
            template<> void destroy(vk::DescriptorSetLayout* ref)   { vkDevice.destroyDescriptorSetLayout(*ref, pAllocator); }
            template<> void destroy(vk::DescriptorPool* ref)        { vkDevice.destroyDescriptorPool(*ref, pAllocator); }
            template<> void destroy(vk::Semaphore* ref)             { vkDevice.destroySemaphore(*ref, pAllocator); }
            template<> void destroy(vk::Fence* ref)                 { vkDevice.destroyFence(*ref, pAllocator); }
        private:
