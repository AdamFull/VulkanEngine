...
namespace utl
{
    ...
    private:
        ...
        static_assert(
            std::is_same_v<key_type, typename _Tree::key_type>,
    ...
    public:
        /* Constructors */
        bimap() = default;
        constexpr bimap(std::initializer_list<_Tree_Node> init) noexcept
        {
    ...
    public:
        ...
        constexpr void insert(_Tree_Node&& node)
        {
        ...
        constexpr void insert(const _Tree_Node& node)
        {
            m_key_tree.emplace_hint(m_key_tree.begin(), std::move(node.first));
            m_value_tree.emplace_hint(m_value_tree.begin(), std::move(node.second));
        }

        constexpr void insert(const key_type& key, const value_type& value)
        {
            m_key_tree.emplace_hint(m_key_tree.begin(), std::move(key));
            m_value_tree.emplace_hint(m_value_tree.begin(), std::move(value));
        }

        constexpr void erase_key(const key_type& key)
        {
            const auto k_itr = m_key_tree.find(key);
        ...
        constexpr void erase_value(const value_type& value)
        {
            const auto v_itr = m_value_tree.find(value);
    ...
    public:
        /* Helper functions */
        constexpr bool has_key(const key_type& key) const
        {
            return m_key_tree.find(key) != m_key_tree.end();
        }

        constexpr bool has_value(const value_type& value) const
        {
            return m_value_tree.find(value) != m_value_tree.end();
        }

        constexpr value_type get_value(const key_type& key) const
        {
        ...
        constexpr key_type get_key(const value_type& value) const
        {
